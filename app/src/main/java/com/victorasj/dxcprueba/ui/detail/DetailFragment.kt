    ...
    private fun UpdateUI(model : DetailViewModel.UiModel){
        when(model){
            is DetailViewModel.UiModel.ContentPhoto -> {
                with(binding){
                    textViewTitle.text = model.photo.title
                    textViewAuthor.text = model.photo.author
                    textViewDate.text = model.photo.date
                    textViewDescription.text = model.photo.description
                    model.photo.image?.let { imageViewPhoto.loadUrl(it) }
                }
            }
        }
    }
